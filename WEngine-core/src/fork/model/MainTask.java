...
public class MainTask extends RecursiveTask<List<String>> {
	...
	private static final long serialVersionUID = 1L;
	   ...
	   public MainTask(String path, String extension)
	   {
	      this.path = path;
	   ...
	   protected List<String> compute()
	   {
	      ...
	      //MainTask tasks to store the subtasks that are going to process the subfolders stored in the folder
	      List<MainTask> tasks = new ArrayList<MainTask>();
	      //Get the content of the folder.
	      File file = new File(path);
	      File content[] = file.listFiles();
	      //For each element in the folder, if there is a subfolder, create a new MainTask object
	      ...
	      if (content != null)
	      {
	         for (int i = 0; i < content.length; i++)
	         {
	            ...
	            {
	               MainTask task = new MainTask(content[i].getAbsolutePath(), extension);
	               task.fork();
	               tasks.add(task);
	            }
	      ...
	      //If the list of the MainTask subtasks has more than 50 elements,
	   ...
	   private void addResultsFromTasks(List<String> list, List<MainTask> tasks)
	   {
	      for (MainTask item : tasks)
	      {
